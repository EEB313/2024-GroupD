...
- Homogeneous mixing of the population, in other words, all infected and susceptible specimens have an equal chance of interaction (Tolles & Luong, 2020). This may not account for ecological barriers or behavioural changes induced by infection. 
...
- Individuals who recover from the disease do not have immunity and thus can be infected again. 

- The parameters used in our model are estimates. For example, we assume the maturation rate for infected and susceptible tadpoles (moving from St to Sf, or It to If) is different because the trade-off between growth and defence will lead infected individuals to allocate more resources to fighting the infection and grow slower (Rauw, 2012).

## Limitations of the Model 

- Our model uses an initial number of zoospores from our TPC. The TPC is created using data based on *L. wilcoxii*/*L. lesueuri*, which are reservoir species, and isolates of *Bd* found in the US. This was an attempt to make a generalized estimate of the number of zoospores for any *Bd* isolate in *L. wilcoxii*/*L. lesueuri*. However, the model cannot account for any other frog species; thus, infection via zoospore spread between species other than *L. wilcoxii*/*L. lesueuri* cannot be modelled. Additionally, each frog species plays a unique role in the disease dynamics. For example, there are differences in zoospore shedding rate and resilience between such species (Lips, 2016).
